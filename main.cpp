...
//Graph Show
        if (complex_graph_on)
            origin_function_one(window, one, size, one_variable_function_complex, current_center_x, current_center_y,
                                x_angle, y_angle, complex_type);
        if (real_graph_on)
            origin_function_two(window, two, size, two_variable_real_function, current_center_x, current_center_y,
                                x_angle, y_angle);
        if (parameter_on) {
            parameter_function_graph(window, three, size, parameter_function1, x_angle, y_angle);
            parameter_function_graph(window, three, size, parameter_function2, x_angle, y_angle);
        }
        int length_axes = 30 * size;

        sf::VertexArray axe1(sf::Lines);
        axe1.append(sf::Vertex(sf::Vector2f(0.0f, 0.0f), sf::Color::Green));
        axe1.append(sf::Vertex(
                sf::Vector2f(-length_axes * std::sin(theta), length_axes * (std::sin(psy)) * (std::cos(theta))),
                sf::Color::Green));

        sf::VertexArray axe2(sf::Lines);
        axe2.append(sf::Vertex(sf::Vector2f(0.0f, 0.0f), sf::Color::Black));
        axe2.append(sf::Vertex(
                sf::Vector2f(-length_axes * std::cos(theta), length_axes * (std::sin(psy)) * (-std::sin(theta))),
                sf::Color::Black));

        sf::VertexArray axe3(sf::Lines);
        axe3.append(sf::Vertex(sf::Vector2f(0.0f, 0.0f), sf::Color::Black));
        axe3.append(sf::Vertex(
                sf::Vector2f(length_axes * std::sin(theta), -length_axes * (std::sin(psy)) * (std::cos(theta))),
                sf::Color::Black));

        sf::VertexArray axe4(sf::Lines);
        axe4.append(sf::Vertex(sf::Vector2f(0.0f, 0.0f), sf::Color::Black));
        axe4.append(sf::Vertex(
                sf::Vector2f(length_axes * std::cos(theta), -length_axes * (std::sin(psy)) * (-std::sin(theta))),
                sf::Color::Black));
        ...
        for (double i = -30 * size; i <= 30 * size; i += 1 * size) {
            axe_x_dot.append(sf::Vertex(sf::Vector2f(i * std::sin(theta), 10 - i * (std::sin(psy)) * (std::cos(theta))),
                                        sf::Color::Black));
            axe_x_dot.append(
                    sf::Vertex(sf::Vector2f(i * std::sin(theta), -10 - i * (std::sin(psy)) * (std::cos(theta))),
                               sf::Color::Black));
        }

        sf::VertexArray axe_y_dot(sf::Lines);
        for (double i = -30 * size; i <= 30 * size; i += 1 * size) {
            axe_y_dot.append(sf::Vertex(sf::Vector2f(i * std::cos(theta), 10 + i * (std::sin(psy)) * (std::sin(theta))),
                                        sf::Color::Black));
            axe_y_dot.append(
                    sf::Vertex(sf::Vector2f(i * std::cos(theta), -10 + i * (std::sin(psy)) * (std::sin(theta))),
                               sf::Color::Black));
        }
